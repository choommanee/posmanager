...
$(document).ready(function() {
    $(document).on('click', '.row_status', function(e) {
        ...
        if (row.hasClass('invoice_link')) {
            ...
            $('#myModal').modal('show');
        } else if(row.hasClass('invoicebill_link')){
            $('#myModal').modal({remote: site.base_url + 'invoice/update_status/' + id});
            $('#myModal').modal('show');
        }
        return false;
    });
...
function formatQty(x) {
    //alert(site.settings.qty_decimals);
...
function formatMoney2(x, d) {
    ...
    var fmoney = accounting.formatMoney(x, symbol, d, site.settings.thousands_sep == 0 ? ' ' : site.settings.thousands_sep, site.settings.decimals_sep, "%s%v");
    return (site.settings.display_symbol == 1 ? site.settings.symbol : '') +
        fmoney +
        (site.settings.display_symbol == 2 ? site.settings.symbol : '');
}
...
function showModal(id,sale_type) {
    $('#myModal').modal({remote: site.base_url + 'sales/modal_view/' + id + '?sale_type=' + sale_type});
    $('#myModal').modal('show');
...
function calculateDiscount(val, amt) {
    ...
    if (val.indexOf("%") !== -1) {
        var pds = val.split("%");
        return formatDecimal((parseFloat(((amt) - parseFloat(val)) )), 4);
    }
...
$(document).ready(function() {
    ...
    $('body').on('click', '.invoice_link td:not(:first-child, :nth-child(6), :nth-last-child(2), :last-child)', function() {
         //alert("test :"+$(this).closest('.invoice_link').attr('sale_type'));
        $('#myModal').modal({remote: site.base_url + 'sales/modal_view/' + $(this).parent('.invoice_link').attr('id')+'?sale_type='+$(this).closest('.invoice_link').attr('sale_type')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
    $('body').on('click', '.invoicebill_link td:not(:first-child, :nth-child(6), :nth-last-child(2), :last-child)', function() {
        // alert("test :"+$(this).closest('.invoicebill_link').attr('sale_type'));
        $('#myModal').modal({remote: site.base_url + 'invoice/modal_view/' + $(this).parent('.invoicebill_link').attr('id')});
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'sales/view/' + $(this).parent('.invoice_link').attr('id');
    });
    $('body').on('click', '.invoice_link2 td:not(:first-child, :last-child)', function() {
        
       // alert("test :"+$(this).closest('tr').attr('sale_type'));
        $('#myModal').modal({remote: site.base_url + 'sales/modal_view/' + $(this).closest('tr').attr('id')+'?sale_type='+$(this).closest('tr').attr('sale_type')});
        $('#myModal').modal('show');
    });
