...
function loadItems() {
    ...
    if (localStorage.getItem('qaitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var unit =  item.row.unit, quantity=item.row.quantity;
            ...
            if(an ==1){
            		tr_From = $('<tr><td colspan="7" class="blue"><b>รายการที่ต้องการแปรรูปสินค้า</b></td></tr>');
            ...
            tr_html += '<td><input name="fquantity[]" type="hidden" class="rid" value="' + quantity + '">'+quantity+'</td>';
            tr_html += '<td>'+unit+'</td>';
            if(an ==1){
            tr_html += '<td><select name="type[]" class="form-contol select rtype" style="width:100%;"><option value="subtraction" selected>'+type_opt.subtraction+'</option></select></td>';
            }else{
            	tr_html += '<td><select name="type[]" class="form-contol select rtype" style="width:100%;"><option value="addition" selected>'+type_opt.addition+'</option></select></td>';
            }
            ...
            if(an ==1){
            	tr_html += '<td>'+(formatQuantity2(parseFloat(quantity)-parseFloat(item_qty)))+'</td>';
            }else{
            	tr_html += '<td>'+(formatQuantity2(parseFloat(quantity)+parseFloat(item_qty)))+'</td>';
            }
            ...
            newTr.html(tr_html);
            if(an ==2){
        		tr_From = $('<tr><td  colspan="7" class="blue"><b>แปรรูปเป็น</b></td></tr>');
        		tr_From.appendTo("#qaTable");
            }
