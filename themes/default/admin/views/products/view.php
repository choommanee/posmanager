<?php defined('BASEPATH') OR exit('No direct script access allowed');
   // print_r($_SESSION);
?>
<?php if ($Owner || $Admin || $_SESSION['group_id']==6) { ?>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                oTable = $('#SlRData').dataTable({
                    ...
                    'sAjaxSource': '<?= admin_url('reports/getSalesReport/?v=1&pid=1&product='.$product->id) ?>',
                    ...
                    "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                        var gtotal = 0, paid = 0, balance = 0,gtotalq=0;
                        for (var i = 0; i < aaData.length; i++) {
                            gtotalq += parseFloat(aaData[aiDisplay[i]][4]);
                            gtotal += parseFloat(aaData[aiDisplay[i]][5]);
                            paid += parseFloat(aaData[aiDisplay[i]][6]);
                            balance += parseFloat(aaData[aiDisplay[i]][7]);
                        }
                        var nCells = nRow.getElementsByTagName('th');
                        nCells[4].innerHTML = currencyFormat(parseFloat(gtotalq));
