...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
       ...
       // alert(pos_settings.item_order);
        if (pos_settings.item_order == 1) {
            sortedItems = _.sortBy(positems, function(o) { return [parseInt(o.category), parseInt(o.id)]; });
      ...
      //  positems.sort();
        ...
        $.each(sortedItems, function () {
            var intQtyunit= localStorage.getItem('intQtyunit');
            var item = this;
            //alert(item.row.pro_code);
            console.log("ข้อมูล Item :");
            console.log(item);
            console.log("ข้อมูล Item promotion:");
            console.log(item.promotion);
            if (item.promotion == "0" ){
                var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
            }else{
                
                var item_id = site.settings.item_addition == 1 ? item.item_id + "_pro" : item.id + "_pro";
            }
            ...
            console.log(rowdata);
            var product_id = item.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
            ...
            var cat_id =item.row.category_id;
          //  alert(cat_id);
            ...
            if(item.row.pro_code !='' && typeof(item.row.pro_code) != "undefined") {
            ...
            //if(item.row.pro_code !='' && typeof(item.row.pro_code) != "undefined"){
                tr_html += '<input class="form-control input-sm rdiscount" name="product_promotion_code[]" type="hidden" id="discount_' + row_no + '" value="' + item.row.pro_code + '">';
            //}
            ...
            if (postax2 = localStorage.getItem('postax2')) {
                $.each(tax_rates, function () {
                    if (this.id == postax2) {
                        ...
                        if (this.type == 1) {
                            invoice_tax = formatDecimal((((total - order_discount) * parseFloat(this.rate)) / (100+parseFloat(this.rate))), 4);
                        }
                    }
                });
            }
        ...
        $('#order_discount_input').val(formatMoney(order_discount));
        console.log(formatMoney(order_discount));
 ...
 * @returns {Boolean}
 ...
 function add_invoice_item(item) {
   ...
   //  alert(intQtyunit);
     console.log(item.promotion);
    // var promotion_id = item.promotion.id;
     if (item.promotion != "0" ){
          //  alert('test');
        console.log("Have A Promotion");
         var item_id = site.settings.item_addition == 1 ? item.item.item_id : item.item.id;
         if (positems[item_id]) {
             if(intQtyunit ==null){
                 var new_qty = parseFloat(positems[item_id].row.qty) + 1;
             }else {
                 var new_qty = parseFloat(positems[item_id].row.qty) + parseFloat(intQtyunit);
             }
             ...
             positems[item_id] = item.item;
             if(intQtyunit==null){
                 var new_qty = 1;
             }else {
                 var new_qty = parseFloat(intQtyunit);
             }
             positems[item_id].row.qty = new_qty;

         }

         var itempro_id = site.settings.item_addition == 1 ? item.promotion.item_id+"_pro" : item.promotion.id+"_pro";
         if (positems[itempro_id]) {
             if(intQtyunit ==null){
                 var new_qty = parseFloat(positems[itempro_id].row.qty) + 1;
             }else {
                 var new_qty = parseFloat(positems[itempro_id].row.qty) + parseFloat(intQtyunit);
             }

             positems[itempro_id].row.base_quantity = new_qty;
             if(positems[itempro_id].row.unit != positems[itempro_id].row.base_unit) {
                 $.each(positems[itempro_id].units, function(){
                     if (this.id == positems[itempro_id].row.unit) {
                         positems[itempro_id].row.base_quantity = unitToBaseQty(new_qty, this);
                     }
                 });
             }
             positems[itempro_id].row.qty = new_qty;

         } else {
             positems[itempro_id] = item.promotion;
             if(intQtyunit==null){
                 var new_qty = 1;
             }else {
                 var new_qty = parseFloat(intQtyunit);
         ...
         console.log("No A Promotion");
         var item_id = site.settings.item_addition == 1 ? item.item_id : item.item.id;
         console.log(positems[item_id]);
         if (positems[item_id]) {
             if(intQtyunit ==null){
                 var new_qty = parseFloat(positems[item_id].row.qty) + 1;
             }else {
            ...
            // console.l(positems[item_id].row);
             positems[item_id].row.qty = new_qty;
         }
     }
