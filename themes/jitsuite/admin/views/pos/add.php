            ...
            <div class="pos">
                ...
                <div id="pos">
                        ...
                        <div id="left-top">
                            ...
                            <div class="no-print">
                                ...
                                <div class="form-group" id="ui">
                                    ...
                                    <div class="input-group">
                                    <?php } ?>
                                    <?php echo form_input('add_item', '', 'class="form-control pos-tip" id="add_item" data-placement="top" data-trigger="focus" placeholder="' . $this->lang->line("search_product_by_name_code") . '"'); ?>
                                    <?php if ($Owner || $Admin || $GP['products-add']) { ?>
                                ...
                                <table id="totalTable"
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right" style="padding: 5px 10px;font-weight:bold;">
                                            <span id="tds" style="display: none;">0.00</span>
                                            <div class="form-group">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right" style="padding:5px 10px 5px 10px; font-size: 40px;border-top: 1px solid #666; border-bottom: 1px solid #333; font-weight:bold; background:#333; color:#ff0d0d;" colspan="2">
                    ...
                    <div id="cp">
                        <div class="example1">
                            <?php
                                // print_r($Promotion_header)
                            if(count($Promotion_header) > 0){
                                echo '<h3 style="margin-top: 0px;">';
                                for($i=0;$i<count($Promotion_header);$i++){
                                    echo $Promotion_header[$i]->pro_name." ( ".SiteHelpers::ConvertDateDbToThai($Promotion_header[$i]->pro_start_date).' - '.SiteHelpers::ConvertDateDbToThai($Promotion_header[$i]->pro_end_date)." )";
                                }
                                echo "</h3>";
                            }
                            ?>
                        ...
                        <div style="float: left;width: 130px;margin-left: 10px;margin-right: 10px;">
                            <div class="col-xs-12" style="padding: 0;margin-bottom: 10px;">
                                <button type="button"  class="btn btn-info open-promotion btn-block" style="height:67px;font-size: 16px;" tabindex="-1">
                                    Promotion<br/>ลด
                                </button>
                            </div>

                            <div class="col-xs-12" style="padding: 0;margin-bottom: 10px;">
                                <button type="button" class="btn btn-success btn-block open-promotion2"  style="height:67px;font-size: 16px;" tabindex="-1">
                                    Promotion<br/>แถม
                                </button>
                            </div>
                            <div class="col-xs-12" style="padding: 0;">
                                <button id="addTypeNum" type="button" class="btn btn-success btn-block"  style="height:67px;font-size: 16px;" tabindex="-1">
                                    จำนวน<br/>
                                </button>
                            </div>
                        </div>

                        <div id="cpinner" style="min-width: 450px; max-width: 45%;margin-left:0px;">
                        ...
                        <div id="payments">
                            <div class="well well-sm well_1">
                                <div class="payment">
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <button name="paid_by[]" id="paid_by_1" type="button" class="btn btn-primary col-md-2 paid_by" tabindex="-1" value="cash">เงินสด</button>
                                            <button name="paid_by[]" id="paid_by_1" type="button" class="btn btn-primary col-md-2 paid_by" value="gift_card">บัตรของขวัญ</button>
                                            <button name="paid_by[]" id="paid_by_1" type="button" class="btn btn-primary col-md-2 paid_by" value="CC">บัตรเครดิต</button>
                                            <button name="paid_by[]" id="paid_by_1" type="button" class="btn btn-primary col-md-2 paid_by" value="Cheque">เช็ค</button>
                                            <button name="paid_by[]" id="paid_by_1" type="button" class="btn btn-primary col-md-2 paid_by" value="other">อื่นๆ</button>
                                            <button name="paid_by[]" id="paid_by_1" type="button" class="btn btn-primary col-md-2 paid_by" value="deposit">deposit</button>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-5">
...
<?php if ($this->input->get('customer')) {?>
        ...
        $('#payment').click(function () {
            ...
            <?php }
            ?>
            var twt = formatDecimal((total) - order_discount + shipping);
            if (count == 1) {
        ...
        $(document).on('click','#addTypeNum',function(){
            console.log('test');
            if($('#add_item').val()==''){
                bootbox.alert('กรุณาใส่จำนวนที่ต้องการ');
                $('#add_item').focus();
                return false;
            }
            if (localStorage.getItem('intQtyunit')) {
                localStorage.removeItem('intQtyunit');

            }
            localStorage.setItem('intQtyunit', $('#add_item').val());
            $('#add_item').val("");
        });
        ...
        $('#category-' + cat_id).addClass('active');
        $('.open-promotion').on('click',function(){
           //alert('1');
            $.ajax({
                type: "get",
                url: "<?=admin_url('pos/ajaxpromotionsall/2');?>",
                dataType: "json",
                success: function (data) {
                    $('#item-list').empty();
                    var newPrs = $('<div></div>');
        ...
        $('.open-promotion2').on('click',function(){
            //alert('1');
            $.ajax({
                type: "get",
                url: "<?=admin_url('pos/ajaxpromotionsall/1');?>",
                dataType: "json",
                success: function (data) {
                    $('#item-list').empty();
                    var newPrs = $('<div></div>');
