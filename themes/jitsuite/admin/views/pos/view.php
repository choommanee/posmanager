        ...
        <div id="receiptData">
            ...
            <div id="receipt-data">
                <div class="text-center">
                    ...
                    <h4 style="text-transform:uppercase;"><?=$biller->company != '-' ? $biller->company : $biller->name;?></h4>
                    <?php
                ...
                echo "ผู้ขาย: " . $created_by->first_name." ".$created_by->last_name . "</p>";
                echo "<p>";
                ...
                <table class="table table-striped table-condensed">
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td class="no-border" style="width: 80%;border: 1px;">' . $r . ': &nbsp;' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') .'('.$this->sma->formatDecimalMoney($row->quantity,2) . ' x '.$this->sma->formatDecimalMoney($row->unit_price,2). ')</td>
                                  <td class="no-border border-bottom text-right" style="width: 20%;border: 1px;">' . $this->sma->formatDecimalMoney($row->subtotal,2) . '</td>
                                  </tr>';
                        ...
                        if ($return_rows) {
                            ...
                            foreach ($return_rows as $row) {
                                ...
                                echo '<tr><td class="no-border">#' . $r . ': &nbsp;&nbsp;' . product_name($row->product_name, ($printer ? $printer->char_per_line : null)) . ($row->variant ? ' (' . $row->variant . ')' : '') .$this->sma->formatQuantity($row->quantity) . ' x '.$this->sma->formatDecimalMoney($row->unit_price,2). '<span class="pull-right">' . ($row->tax_code ? '*'.$row->tax_code : '') . '</span></td>
                                      <td class="no-border border-bottom text-right">' . $this->sma->formatDecimalMoney($row->subtotal,2) . '</td>
                                      </tr>';
                                //echo '<tr><td class="no-border border-bottom">' . $this->sma->formatQuantity($row->quantity) . ' x '.$this->sma->formatDecimalMoney($row->unit_price,2).'</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td></tr>';
                    ...
                    <tfoot>
                        <tr>
                            <th><?=lang("total");?></th>
                            <th class="text-right"><?=$this->sma->formatMoney($return_sale ? (($inv->total)+($return_sale->total + $return_sale->product_tax)) : ($inv->total));?></th>
                        </tr>
                        <?php
                       // if ($inv->order_tax != 0) {
