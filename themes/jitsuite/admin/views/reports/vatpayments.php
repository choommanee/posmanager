...
<script>
    $(document).ready(function () {
        ...
        oTable = $('#PayRData').dataTable({
            ...
            "aLengthMenu": [[10, 50, -1], [10, 25,50,20, 50,50,50, 100, "<?= lang('all') ?>"]],
            "iDisplayLength": <?= $Settings->rows_per_page ?>,
            'bProcessing': true, 'bServerSide': true,
            'sAjaxSource': '<?= admin_url('reports/getVatPaymentsReport/?v=1' . $v) ?>',
            ...
            "aoColumns": [{"mRender": fld}, null,  {"mRender": paid_by},null,null,null, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": row_status}, {"bVisible": false}],
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
                    if (aaData[aiDisplay[i]][8] == 'sent' || aaData[aiDisplay[i]][8] == 'returned'){
                        ...
                        totalVat -= parseFloat(aaData[aiDisplay[i]][7]);
                        }else{
                        total += parseFloat(aaData[aiDisplay[i]][6]);
                        totalVat += parseFloat(aaData[aiDisplay[i]][7]);
                    }
                ...
                nCells[7].innerHTML = currencyFormat(parseFloat(totalVat));
            }
        }).fnSetFilteringDelay().dtFilter([
            {column_number: 0, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 1, filter_default_label: "[<?=lang('sale_ref');?>]", filter_type: "text", data: []},
            {column_number: 2, filter_default_label: "[<?=lang('paid_by');?>]", filter_type: "text", data: []},
            {column_number: 3, filter_default_label: "[ชื่อผู้ซื้อสินค้า]", filter_type: "text", data: []},
        ], "footer");
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-money"></i><?= lang('vatpayments_report'); ?> <?php
            if ($this->input->post('start_date')) {
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div id="form">
                    ...
                    <?php echo admin_form_open("reports/vatpayments"); ?>
                    <div class="row">
                        <div class="col-sm-4">
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#pdf').click(function (event) {
            ...
            window.location.href = "<?=admin_url('reports/getVatPaymentsReport/pdf/?v=1'.$v)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=admin_url('reports/getVatPaymentsReport/0/xls/?v=1'.$v)?>";
            return false;
        });
