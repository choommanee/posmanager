...
class Site extends CI_Model
{
    ...
    public function getCompanyByDefualt() {
        $q = $this->db->get_where('companies', array('id' => "3"), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getAllVatWarehouses() {
        $q = $this->db->get('warehouses');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function modal_js() {
        //echo $this->data['assets'] . 'js/modal.js';
    ...
    public function syncProductQty($product_id, $warehouse_id) {
        $balance_qty = $this->getBalanceQuantity($product_id);//ดึงข้อมูลรายการคงเหลือ
        $wh_balance_qty = $this->getBalanceQuantity($product_id, $warehouse_id);//ดึงข้อมูลรายการคงเหลือของคลังนั้นๆ
    ...
    //คำนวณต้นทุนสินค้า
    //รหัสสินค้า  รหัสคลังสินค้า ราคาขายต่อหน่วย ราคาขาย จำนวน ชื่อสินค้า ออฟชั่น และจำนวน
    public function calculateCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $product_name, $option_id, $item_quantity) {
       // echo $product_id;
       //ดึงข้อมูลรายการสั่งซื้อทั้งหมดที่มีสินค้านี้ในคลังนี้
        $pis = $this->getPurchasedItems($product_id, $warehouse_id, $option_id);
        $real_item_qty = $quantity;///กำหนดจำนวนจริง = จำนวนที่ได้ความต้องการตรวจสอบ
        $quantity = $item_quantity;/// จำนวนที่ขายไป
        $balance_qty = $quantity;// จำนวนคงเหลือ
        foreach ($pis as $pi) {//ตรวจสอบการทำงาน
            $cost_row = NULL;
            //กรณีที่รายการสั่งซื้อไม่เป็นค่าว่าง และ จำนวนคงเหลือ น้อยกว่า จำนวนที่ขาย และจำนวนที่ขายมากกว่า 0
            if (!empty($pi) && $balance_qty <= $quantity && $quantity > 0) {
                //กำหนดราคาต้นทุนของหน่วยขาย เท่ากับราคาต้นทุนที่ซื้อ หรือไม่มีคำนวณจาก ราคาขาย

                if($pi->unit_cost > 0 || !empty($pi->item_tax)){
                    $purchase_unit_cost = $pi->unit_cost ? $pi->unit_cost : ($pi->net_unit_cost + ($pi->item_tax / $pi->quantity));
                }else{
                    $purchase_unit_cost = $pi->unit_cost;
                }


                // กรณีที่จำนวนในการสั่งซื้อมากกว่าจำนวนที่ขาย
                if ($pi->quantity_balance >= $quantity && $quantity > 0) {
                    //จำนวนคงเหลือ = จำนวนสั่งซื้อ - จำนวนที่ขาย
                    $balance_qty = $pi->quantity_balance - $quantity;

                    //รายละเอียดค่าใช้จ่าย
                    $cost_row = array('date' => date('Y-m-d'),
                    ...
                    $quantity = 0;
                    //กรณีที่รายการยังไม่ครบและจำนวนที่ขายมากกว่าจำนวนการสั่งซื้อ
                } elseif ($quantity > 0) {
                    // จำนวนที่ขาย = จำนวนที่ขาย - รายการสั่งซื้อ
                    $quantity = $quantity - $pi->quantity_balance;
                    //echo "<br/>";
                    // จำนวนที่เหลือ = จำนวนสั่งซื้อ
                    $balance_qty = $quantity;
                    $cost_row = array('date' => date('Y-m-d'),
                                      'product_id' => $product_id,
                                      'sale_item_id' => 'sale_items.id',
    ...
    public function getPurchasedItems($product_id, $warehouse_id, $option_id = NULL) {
        //เรียงตามการตั้งค่าวิธีการคิดต้นทุน และวิธีการคิดการขาย FIFO=1 ,LIFO=2,Average=3
        $orderby = ($this->Settings->accounting_method == 1) ? 'asc' : 'desc';
    ...
    // รายการสินค้าที่ทำเป็นเซ็คคอมโบ้
    ...
    public function item_costing($item, $pi = NULL) {
        ...
        // ตรวจสอบการคำนวณบัญชีและการตั้งค่าขายเกินจำนวน
        if ($this->Settings->accounting_method != 2 && !$this->Settings->overselling) {
            ...
            if ($this->getProductByID($item['product_id'])) {
                if ($item['product_type'] == 'standard') {
                    ...
                    $cost = $this->calculateCost($item['product_id'], $item['warehouse_id'], $item['net_unit_price'], $item['unit_price'], $this->sma->formatDecimal($item['quantity'],2), $item['product_name'], $item['option_id'], $this->sma->formatDecimal($item_quantity,2));
...
//การคิดต้นทุน
    ...
    public function syncQuantity($sale_id = NULL, $purchase_id = NULL, $oitems = NULL, $product_id = NULL) {
        ...
        } elseif ($oitems) { //กรณีที่มีการ update การขาย

            foreach ($oitems as $item) {  // ลิสต์รายการทั้งหมด
                if (isset($item->product_type)) { // หากมีประเภท
                    if ($item->product_type == 'standard') {//กรณีที่เป็นสินค้าปกติ
                        $this->syncProductQty($item->product_id, $item->warehouse_id);
                        if (isset($item->option_id) && !empty($item->option_id)) {
                            $this->syncVariantQty($item->option_id, $item->warehouse_id, $item->product_id);
                        }
    ...
    public function calculateOrderTaxSale($order_tax_id = NULL, $amount) {
        if ($this->Settings->tax2 != 0 && $order_tax_id) {
            if ($order_tax_details = $this->site->getTaxRateByID($order_tax_id)) {
                if ($order_tax_details->type == 1) {
                    return $this->sma->formatDecimal((($amount * $order_tax_details->rate) / (100+$order_tax_details->rate)), 4);
                } else {
                    return $this->sma->formatDecimal($order_tax_details->rate, 4);
                }
            }
