...
class Pos_model extends CI_Model
{
    ...
    public function fetch_promotions($promotion_type, $limit, $start, $subcategory_id = NULL, $brand_id = NULL)
    {
        $this->db->select(' a.*,b.*,c.pro_name,c.pro_start_date,c.pro_end_date,c.pro_type')
            ->from('products a ')->join(' promotions_data b','a.id=b.prod_product_id' ,'inner')
            ->join('promotions_header c','b.pro_id=c.pro_id','inner');
        if($promotion_type!=''){
            if($promotion_type==1){
                $query = $this->db->get_where("promotions_header "," ('".date('Y-m-d')."' BETWEEN c.pro_start_date AND c.pro_end_date) and c.pro_type in(0,1)");
            }else{
                $query = $this->db->get_where("promotions_header "," ('".date('Y-m-d')."' BETWEEN c.pro_start_date AND c.pro_end_date) and c.pro_type in(2,3)");
            }
        }else{
            $query = $this->db->get_where("promotions_header "," ('".date('Y-m-d')."' BETWEEN c.pro_start_date AND c.pro_end_date)");
        }
    ...
    public function fetch_promotion_header()
    {
        $this->db->select(' a.pro_name,a.pro_start_date,a.pro_end_date,a.pro_type')->from('promotions_header a ');
            $query = $this->db->get_where("promotions_header "," ('".date('Y-m-d')."' BETWEEN a.pro_start_date AND a.pro_end_date)");
    ...
    public function products_count_promotion(){
        $this->db->select(' a.*,b.*,c.pro_name,c.pro_start_date,c.pro_end_date,c.pro_type')
            ->from('products a ')->join(' promotions_data b','a.id=b.prod_product_id' ,'inner')
            ->join('promotions_header c','b.pro_id=c.pro_id','inner');
        $query =$this->db->get_where("promotions_header "," ('".date('Y-m-d')."' BETWEEN c.pro_start_date AND c.pro_end_date)");
    ...
    {
        $this->db->select('sma_products.*, sma_warehouses_products.quantity, sma_categories.id as category_id, sma_categories.name as category_name')
        ...
        $q = $this->db->get_where("sma_products", array('sma_products.code' => $code));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getWHProductPromotion($code, $warehouse_id)
    {
       /* $this->db->select('sma_products.*, sma_warehouses_products.quantity, sma_categories.id as category_id, sma_categories.name as category_name')
            ->join('sma_promotions_data','sma_promotions_data.prod_product_id =sma_products.id','inner')
            ->join('sma_promotions_header','sma_promotions_header.pro_id =sma_promotions_data.pro_id','inner')
            ->join('sma_warehouses_products', 'sma_warehouses_products.product_id=sma_products.id', 'left')
            ->join('sma_categories', 'sma_categories.id=sma_products.category_id', 'left');
        //->group_by('products.id');
        $this->db->get_where("sma_promotions_header "," ('".date('Y-m-d')."' BETWEEN sma_promotions_header.pro_start_date AND sma_promotions_header.pro_end_date)");
        $q = $this->db->get_where("sma_products", array('sma_products.code' => $code));*/

        $sql ="Select sma_products.*, sma_warehouses_products.quantity, sma_categories.id as category_id, sma_categories.name as category_name,sma_promotions_data.*,sma_promotions_header.*
                    from sma_products
                    Inner join sma_promotions_data on(sma_promotions_data.prod_product_id =sma_products.id)
                    Inner join sma_promotions_header on(sma_promotions_header.pro_id =sma_promotions_data.pro_id)
                    ...
                    where sma_products.code ='{$code}' and ('".date('Y-m-d')."' BETWEEN sma_promotions_header.pro_start_date AND sma_promotions_header.pro_end_date)
                    ";
    ...
    public function getProductOptions($product_id, $warehouse_id, $all = NULL)
    {
        ...
        $this->db->select('sma_product_variants.id as id, sma_product_variants.name as name, sma_product_variants.price as price, sma_product_variants.quantity as total_quantity, FWPV.quantity as quantity', FALSE)
            ->join($wpv, 'FWPV.option_id=sma_product_variants.id', 'left')
            //->join('warehouses', 'warehouses.id=product_variants.warehouse_id', 'left')
            ->where('sma_product_variants.product_id', $product_id);
            //->group_by('sma_product_variants.id');
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = NULL)
    {
        ...
        if ($this->db->insert('sales', $data)) {
            $sale_id = $this->db->insert_id();//บันทึกข้อมูลการขาย

            foreach ($items as $item) { // ข้อมูลไอเเท็มที่ต้องบันทึก

                $item['sale_id'] = $sale_id;  // เซลล์ไอดี
                $this->db->insert('sale_items', $item); // บันทึกข้อมูลไอเท็มที่ละไอเท็ม
                $sale_item_id = $this->db->insert_id();
                // หากสถานะเป็น Completed และมีรหัสสินค้า
                if ($data['sale_status'] == 'completed' && $this->site->getProductByID($item['product_id'])) {

                    $item_costs = $this->site->item_costing($item); //ตรวจสอบต้นทุนของสินค้าแต่ละรายการ
    ...
    public function getAllInvoiceItems($sale_id)
    {
        ...
        $q = $this->db->get_where('sale_items', array('sale_id' => $sale_id,'item_promotion_code'=>''));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
        ...
        if ($this->pos_settings->item_order == 0) {
            $this->db->select('sale_items.*,sma_promotions_header.pro_code as category_id, sma_promotions_header.pro_name as category_name, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, products.details as details, products.hsn_code as hsn_code')
                ->join('products', 'products.id=sale_items.product_id', 'left')
                ->join('tax_rates', 'tax_rates.id=sale_items.tax_rate_id', 'left')
                ->join('sma_promotions_header', 'sma_promotions_header.pro_code=sale_items.item_promotion_code', 'inner')
                ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
                ->group_by('sale_items.id')
                ->order_by('id', 'asc');
        } elseif ($this->pos_settings->item_order == 1) {
            $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate, product_variants.name as variant, sma_promotions_header.pro_code as category_id, sma_promotions_header.pro_name as category_name, products.details as details, products.hsn_code as hsn_code')
                ->join('tax_rates', 'tax_rates.id=sale_items.tax_rate_id', 'left')
                ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
                ->join('products', 'products.id=sale_items.product_id', 'left')
                ->join('categories', 'categories.id=products.category_id', 'left')
    ...
    public function getRegisterSales($date, $user_id = NULL)
    {
        ...
        $this->db->where('payments.created_by', $user_id)->where('payments.pos_paid >','0');
