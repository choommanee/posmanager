...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $limit = 25)
    {
    ...
    {
        $this->db->select('sale_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name,units.name unit_name, tax_rates.rate as tax_rate, products.image, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
       ...
       // die();
        if ($this->db->insert('sales', $data)) {
            ...
            if($this->input->get('sale_type')=='1'){
                if ($this->site->getReference('LC') == $data['reference_no']) {
                    $this->site->updateReference('LC');
                }
            }else{
                if ($this->site->getReference('LV') == $data['reference_no']) {
                    $this->site->updateReference('LV');
                }
            }
