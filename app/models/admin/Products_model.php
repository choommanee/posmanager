...
class Products_model extends CI_Model
{
    ...
    public function getProductToNewItemOptions($pid)
    {
        $q = $this->db->get_where('product_variants', array('product_id' => $pid));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getProductOptionsWithWH($pid)
    {
        $this->db->select($this->db->dbprefix('product_variants') . '.*, ' . $this->db->dbprefix('warehouses') . '.name as wh_name, ' . $this->db->dbprefix('warehouses') . '.id as warehouse_id, ' . $this->db->dbprefix('warehouses_products_variants') . '.quantity as wh_qty')
            ...
            ->join('sma_warehouses', 'sma_warehouses.id=sma_warehouses_products_variants.warehouse_id', 'left')
            //->group_by(array('' . $this->db->dbprefix('product_variants') . '.id', '' . $this->db->dbprefix('warehouses_products_variants') . '.warehouse_id'))
            ->order_by('sma_product_variants.id');
        $q = $this->db->get_where('sma_product_variants', array('sma_product_variants.product_id' => $pid, 'sma_warehouses_products_variants.quantity !=' => NULL));
    ...
    public function getAllWarehousesWithPQ($product_id)
    {
        $this->db->select('' . $this->db->dbprefix('warehouses') . '.*, ' . $this->db->dbprefix('warehouses_products') . '.quantity, ' . $this->db->dbprefix('warehouses_products') . '.rack')
            ->join('sma_warehouses_products', 'sma_warehouses_products.warehouse_id=sma_warehouses.id', 'left')
            ->where('sma_warehouses_products.product_id', $product_id);
    ...
    public function getProductNames($term, $limit = 5)
    {
        $this->db->select('' . $this->db->dbprefix('products') . '.id, code, ' . $this->db->dbprefix('products') . '.name as name, ' . $this->db->dbprefix('products') . '.price as price, ' . $this->db->dbprefix('product_variants') . '.name as vname')
            ->where("type != 'combo' AND "
                . "(" . $this->db->dbprefix('products') . ".name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR cf1 LIKE '%" . $term . "%' OR
    ...
    public function getProductsForOnNewItemPrinting($term,$warehouse_id=null, $limit = 5)
    {
        ...
        ->join("( SELECT product_id, quantity, rack from {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id', 'left')
    ...
    public function getTonewItems($adjustment_id,$warehouse_id)
    {
        $this->db->select('adjustment_items.*,'.$this->db->dbprefix('products') . ".id as products_id, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, {$this->db->dbprefix('units')}.name as unit, wp.rack as rack, alert_quantity", FALSE)
        ->join('products', 'products.id=adjustment_items.product_id', 'left')
        ->join("( SELECT product_id, quantity, rack from {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id', 'left')
        ->join('categories', 'products.category_id=categories.id', 'left')
        ->join('units', 'products.unit=units.id', 'left')
    ...
    public function addIntoNewItem($data, $products)
    {
        if ($this->db->insert('adjustments', $data)) {
            ...
            if ($this->site->getReference('QN') == $data['reference_no']) {
                $this->site->updateReference('QN');
            }
            return true;
        }
        return false;
    ...
    public function updateTonewitem($id, $data, $products)
    {
