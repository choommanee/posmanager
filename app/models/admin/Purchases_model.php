...
class Purchases_model extends CI_Model
{
    ...
    {
        $this->db->where(" (name LIKE '%" . $term . "%' OR code LIKE '%" . $term . "%' OR cf1 LIKE '%" . $term . "%' OR  concat(name, ' (', code, ')') LIKE '%" . $term . "%')");
        $this->db->limit($limit);
    ...
    {
        $this->db->select('purchase_items.*, tax_rates.code as tax_code, tax_rates.name as tax_name, tax_rates.rate as tax_rate,units.name unit_name, products.unit, products.details as details, product_variants.name as variant, products.hsn_code as hsn_code')
