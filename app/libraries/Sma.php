...
class Sma
{
    ...
    public function formatDecimalMoney($number, $decimals = null)
    {
        if (!is_numeric($number)) {
            return null;
        }
    ...
    public function generatecustom_pdf($content, $name = 'download.pdf', $output_type = null, $totalRec,$peperType=null)
    {

        $this->load->library('tec_mpdf2', '', 'pdf');

        return $this->pdf->generate($content, $name, $output_type, $footer, $margin_bottom, $header, $margin_top, $orientation,$totalRec,$peperType);
    }
