...
class Pos extends MY_Controller
{
    ...
    public function getSales($warehouse_id = NULL)
    {
        ...
        $detail_link2 = anchor('admin/sales/modal_view/$1?sale_type=1', '<i class="fa fa-file-text-o"></i> ' . lang('sale_details_modal'), 'data-toggle="modal" data-target="#myModal"');
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $date = date('Y-m-d H:i:s'); // วันที่ขาย
            $warehouse_id = $this->input->post('warehouse'); // Warehouse ที่ใช้ในการขาย
            $customer_id = $this->input->post('customer'); // ลูกค้า
            $biller_id = $this->input->post('biller');  // รหัสบิล
            $total_items = $this->input->post('total_items');   // จำนวนสินค้า
            $sale_status = 'completed';  // สถานะการขาย
            $payment_status = 'due'; // สถานะการจ่ายเงิน
            $payment_term = 0; // เครดิตเทอม
            $due_date = date('Y-m-d', strtotime('+' . $payment_term . ' days')); //วันครบกำหนด
            $shipping = $this->input->post('shipping') ? $this->input->post('shipping') : 0; // ค่าใช้จ่ายในการขนส่ง
            $customer_details = $this->site->getCompanyByID($customer_id); // รายละเอียดลูกค้า
            // ข้อมูลลูกค้า หากมีบริษัทจะใส่ชื่อบริษัทเข้ามาให้
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_promotion_code = $_POST['product_promotion_code'][$r];
                ...
                //ตรวนสอบว่ามีการสร้างรหัสสินค้า ราคาจริง ราคาต่อหน่วย และ จำนวนหรือไม่
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    $product = array(
                        ...
                        'item_promotion_code'=> $item_promotion_code,
            ...
            $order_tax = ((($total-$total_discount)*7)/107);
            //echo "<br/>";
        ...
        if ($this->form_validation->run() == TRUE && !empty($products) && !empty($data)) { // กรณีมีการบันทึก
            if ($suspend) { //บันทึกพักบิล
            ...
            if (($sid || $duplicate_sale) && $inv_items) {   //ดึงรายการจากเหตุการณ์ พักบิล รายการ Copy รายการ Invoice
                    // krsort($inv_items);
            ...
            // กรณีที่มาจากการพิมพ์ หลังจากการบันทึก
            ...
            //รายการโปรโมชั่น
            $promotionHeader = $this->pos_model->fetch_promotion_header();
            $this->data['Promotion_header'] =$promotionHeader;
    ...
    public function getProductDataByCode($code = NULL, $warehouse_id = NULL)
    {
        ...
        //$rowDataPromotion = $this->pos_model->getWHProductPromotion($code, $warehouse_id);
        //print_r($rowDataPromotion);
        $promotion_data = $this->getProductPromotionCode($code);

        //print_r($promotion_data);
        if($promotion_data['row']->pro_type==0 or $promotion_data['row']->pro_type==1 ){
           // echo $promotion_data['row']->pro_name;
        }else{
           // echo $promotion_data['row']->pro_name;
        }
        $option = false;
        if ($row) {
            ...
            $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
            //echo 'test';
            //print_r($promotion_data['row']);
            //var_dump($promotion_data);
            if(!empty($promotion_data) and ($promotion_data['row']->pro_type==0 or $promotion_data['row']->pro_type==1) ){
              //  print_r($promotion_data['row']);
               // echo "test have Promotions";
                $pr['promotion']=1;
                $pr['id']=str_replace(".", "", microtime(true));
                $pr['item'] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
                $pr['promotion'] = $promotion_data;
    ...
    public function getProductPromotionCode($code = NULL, $warehouse_id = NULL)
    {
        $this->sma->checkPermissions('index');
        if ($this->input->get('code')) {
            $code = $this->input->get('code', TRUE);
        ...
        $customer_group = $this->site->getCustomerGroupByID($customer->customer_group_id);
        //$row = $this->pos_model->getWHProduct($code, $warehouse_id);
        $row = $this->pos_model->getWHProductPromotion($code, $warehouse_id);
       // print_r($rowDataPromotion);
        $option = false;
        if ($row) {
            unset($row->cost, $row->details, $row->product_details, $row->image, $row->barcode_symbology, $row->cf1, $row->cf2, $row->cf3, $row->cf4, $row->cf5, $row->cf6, $row->supplier1price, $row->supplier2price, $row->cfsupplier3price, $row->supplier4price, $row->supplier5price, $row->supplier1, $row->supplier2, $row->supplier3, $row->supplier4, $row->supplier5, $row->supplier1_part_no, $row->supplier2_part_no, $row->supplier3_part_no, $row->supplier4_part_no, $row->supplier5_part_no);
            ...
            $row->unit = $row->sale_unit ? $row->sale_unit : $row->unit;
            $row->comment = '';
            $row->category_id= $row->pro_code;
            $row->category_name= $row->pro_name;
            $combo_items = false;
            if ($row->type == 'combo') {
                $combo_items = $this->pos_model->getProductComboItems($row->id, $warehouse_id);
            }
            ...
            $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
             ...
             $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->pro_name, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
    ...
    public function ajaxpromotion($promotion_type=null,$category_id = NULL, $brand_id = NULL)
    {
        ...
        $config = array();
        $config["base_url"] = base_url() . "pos/ajaxpromotion";
        $config["total_rows"] = $this->pos_model->products_count_promotion();
        ...
        //print_r($config);
        $products = $this->pos_model->fetch_promotions($promotion_type, $config["per_page"], $page, $subcategory_id, $brand_id);
        //print_r($products);
        $pro = 1;
        $prods = '<div>';
        $tmpProname='';
        if (!empty($products)) {
            foreach ($products as $product) {
               ...
               // print_r($product);
                if($product->pro_name <> $tmpProname){
                    $prods.="<h3 style='color: red;margin-top: 0px;'>{$product->pro_name} ต้งแต่วันที่ ".SiteHelpers::ConvertDateDbToThai($product->pro_start_date)." ถึงวันที่  ".SiteHelpers::ConvertDateDbToThai($product->pro_end_date)."</h3>";
                }
                if($product->pro_type=='0' || $product->pro_type=='1'){
                    $txtPromotion = "ซื้อ {$product->prod_qty} แถม {$product->prod_free} ";
                }else{
                    $txtPromotion = "ซื้อ {$product->prod_qty} ลด {$product->prod_free}";
                }
                $prods .= "<button style=\"height:200px;\" id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" 
                                class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\">
                                <img src=\"" . base_url() . "assets/uploads/thumbs/" . $product->image . "\" alt=\"" . $product->name . "\" class='img-rounded' />
                                <span style='float: left;'>" . $product->pro_type.character_limiter($product->name, 40) . "</span>
                                <span style='float: left;'><b>" . character_limiter($txtPromotion, 40) . "</b></span>
                            </button>";

                $pro++;
                $tmpProname = $product->pro_name;
            }
        }else{
            $prods .= "<h3>ไม่พบโปรโมชั่น</h3>";
        }
         $prods .= "</div>";
    ...
    public function ajaxpromotionsall($promotion_type = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        $products = $this->ajaxpromotion($promotion_type, FALSE,FALSE);

        if (!($tcp = $this->pos_model->products_count_promotion(FALSE, FALSE, FALSE))) {
            $tcp = 0;
        }
