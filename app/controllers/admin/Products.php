...
class Products extends MY_Controller
{
    ...
    function index($warehouse_id = NULL)
    {
        //echo "test";
        //die();
        //ตรวจสอบสิทธิ์ในการใช้งานโปรแกรม
    ...
    function add($id = NULL)
    {
        ...
        $this->form_validation->set_rules('slug', lang("slug"), 'is_unique[products.slug]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'reduce_point' => $this->input->post('reduce_point'),
  ...
  /*   function qa_suggestions_tonewItem($warehouse_id=null)
    {
        $term = $this->input->get('term', TRUE);
        if (strlen($term) < 1 || !$term) {
            die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . admin_url('welcome') . "'; }, 10);</script>");
        ...
        $rows = $this->products_model->getProductsForOnNewItemPrinting($term,$warehouse_id);
        if ($rows) {
            foreach ($rows as $row) {
                $variants = $this->products_model->getProductToNewItemOptions($row->id);
                $pr[] = array('id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'code' => $row->code, 'name' => $row->name, 'price' => $row->price, 'qty' => 1, 'variants' => $variants);
            }
            $this->sma->send_json($pr);
        } else {
     ...
     */
    public function qa_suggestions_tonewItem($warehouse_id=null)
    {
        $term = $this->input->get('term', true);
        ...
        $rows = $this->products_model->getProductsForOnNewItemPrinting($sr,$warehouse_id);
        if ($rows) {
            foreach ($rows as $row) {
                $row->qty = 1;
    ...
    /* ----------------------------------------------------------------------------- */
    // ปรับปรุงจำนวนสินค้า หน้ารายการปรับปรุง

    function quantity_tonewitem($warehouse_id = NULL)
    {
        ...
        $meta = array('page_title' => lang('quantity_tonewitem'), 'bc' => $bc);
        $this->page_construct('products/quantity_tonewitem', $meta, $this->data);
    }
    ...
    function getadjustments($warehouse_id = NULL)
    {
        ...
        $this->datatables
            ...
            ->where('adjustments.type_adjust <>', 2)
    ...
    function gettonewitem($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('adjustments');

        $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_adjustment") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('products/delete_tonewitem/$1') . "'>"
                . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";
                ...
                $this->datatables->add_column("Actions", "<div class='text-center'><a href='" . admin_url('products/edit_tonewitem/$1') . "' class='tip' title='" . lang("edit_tonewitem") . "'><i class='fa fa-edit'></i></a> " . $delete_link . "</div>", "id");

                echo $this->datatables->generate();

    }

    public function view_tonewitem($id)
    {
    ...
    // รายการเพิ่มการแปรรูปสินค้า
    function add_tonewitem($count_id = NULL)
    {
        // ตรวจสอบการได้สิทธิ์การทำงาน
        $this->sma->checkPermissions('adjustments', true);
        $this->form_validation->set_rules('warehouse', lang("warehouse"), 'required');

        // ตรวจสอบการทำงาน ผ่าน จะเป็นการเพิ่มรายการแปรรูปสินค้า
        if ($this->form_validation->run() == true) {
            //แก้ไขวันที่ได้หากเป็น Owner และ Admin
            if ($this->Owner || $this->Admin) {
            ...
            // รหัสอ้างอิง
            $reference_no = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('QN');
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $product_id = $_POST['product_id'][$r];  //รหัสสินค้า
                $type = $_POST['type'][$r]; // ประเภท เพิ่มหรือลด
                $quantity = $_POST['quantity'][$r]; // จำนวน
                $serial = $_POST['serial'][$r];//กรรณีมีการบันทึกเป็น Serial
        ...
        if ($this->form_validation->run() == true && $this->products_model->addIntoNewItem($data, $products)) {
            $this->session->set_userdata('remove_qals', 1);
            $this->session->set_flashdata('message', lang("quantity_adjusted"));
            admin_redirect('products/quantity_tonewitem');
        } else {
            // หน้าบันทึกรายการแปรรูปสินค้า
            if ($count_id) {
            ...
            $this->page_construct('products/add_tonewitem', $meta, $this->data);
    ...
    function edit_tonewitem($id)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateAdjustment($id, $data, $products)) {
            $this->session->set_userdata('remove_qals', 1);
            $this->session->set_flashdata('message', lang("edit_tonewitem"));
            admin_redirect('products/quantity_tonewitem');
        } else {

            $inv_items = $this->products_model->getTonewItems($id,$adjustment->warehouse_id);
            // krsort($inv_items);
            $c = rand(100000, 9999999);
            ...
            $meta = array('page_title' => lang('edit_tonewitem'), 'bc' => $bc);
            $this->page_construct('products/edit_tonewitem', $meta, $this->data);
