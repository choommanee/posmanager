...
class Purchases extends MY_Controller
{
    ...
    public function modal_view($purchase_id = null)
    {
        ...
        $this->data['biller'] = $this->site->getCompanyByDefualt();
           ...
           // if ($this->Owner || $this->Admin) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity)) {
                    ...
                    if($_POST['subtotal'][$r] <>""){
                        $subtotal = $this->sma->formatDecimal(str_replace(",","",$_POST['subtotal'][$r]));
                    }else{
                    ...
                    $product = array(
                        ...
                        'subtotal' => $this->sma->formatDecimal($subtotal,2),
                        'expiry' => $item_expiry,
                    ...
                    $total += $this->sma->formatDecimal(($item_net_cost * $item_unit_quantity), 2);
                }
            }
            ...
            if($this->input->post('shipping_pay_type')=='1') {
                $order_tax = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total +$this->sma->formatDecimal($shipping) - $total_discount));
            }else{
                $order_tax = $this->site->calculateOrderTax($this->input->post('order_tax'), ($total - $total_discount));
            }
            ...
            $data = array('reference_no' => $reference,
                ...
                'due_date' => $due_date,
                'type_vat' => $this->input->post('type_vat'),
            );
    ...
    public function edit($id = null)
    {
               ...
               // print_r($_POST);
                ...
                if (isset($item_code) && isset($real_unit_cost) && isset($unit_cost) && isset($item_quantity) && isset($quantity_balance)) {
                   ...
                   // if($_POST['subtotal'][$r] <>""){
                    ...
                    $items[] = ($item+$gst_data);
                    $total += $this->sma->formatDecimal($subtotal,2);
                }
            }
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            foreach ($inv_items as $item) {
                ...
                $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",
                    'row' => $row, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options,'subtotal'=>$row->subtotal);
                $c++;
            }
           // print_r($row);
