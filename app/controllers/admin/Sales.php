...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $detail_link = anchor('admin/sales/view/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-file-text-o"></i> ' . lang('sale_details'));
        $duplicate_link = anchor('admin/sales/add?sale_id=$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-plus-circle"></i> ' . lang('duplicate_sale'));
        $payments_link = anchor('admin/sales/payments/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-money"></i> ' . lang('view_payments'), 'data-toggle="modal" data-target="#myModal"');
        $add_payment_link = anchor('admin/sales/add_payment/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-money"></i> ' . lang('add_payment'), 'data-toggle="modal" data-target="#myModal"');
        $packagink_link = anchor('admin/sales/packaging/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-archive"></i> ' . lang('packaging'), 'data-toggle="modal" data-target="#myModal"');
        $add_delivery_link = anchor('admin/sales/add_delivery/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-truck"></i> ' . lang('add_delivery'), 'data-toggle="modal" data-target="#myModal"');
        $email_link = anchor('admin/sales/email/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-envelope"></i> ' . lang('email_sale'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link = anchor('admin/sales/edit/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-edit"></i> ' . lang('edit_sale'), 'class="sledit"');
        $pdf_link = anchor('admin/sales/pdf/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-file-pdf-o"></i> ' . lang('download_pdf'));
        $return_link = anchor('admin/sales/return_sale/$1?sale_type='.$this->input->get('sale_type'), '<i class="fa fa-angle-double-left"></i> ' . lang('return_sale'));
        ...
        if ($warehouse_id) {
            ...
            if($this->input->get('sale_type')==1){
                ...
                $this->datatables
                    ...
                    ->from('sales')
                    ->where('payment_status in (\'pending\',\'partial\',\'paid\')')
                    ->where('warehouse_id', $warehouse_id);

            }elseif($this->input->get('sale_type')==2){
                $sale_type_txt = array("paid");
                $this->datatables
                    ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, reference_no, biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                    ->from('sales')
    ...
    public function combine_pdf($sales_id)
    {
        ...
        foreach ($sales_id as $id) {
            ...
            if($this->input->post('sale_type')==1){
                if($this->input->post('pepperType')==1){
                    $peperType=1;
                }elseif($this->input->post('pepperType')==2){
                    $peperType=2;
                    //$peperType =2;
                }else{
                    $peperType=1;
                }
            }elseif($this->input->post('sale_type')==2){
                if($this->input->post('pepperType')==1){
                    $peperType=1;
                }elseif($this->input->post('pepperType')==2){
                    $peperType =2;
            ...
            $this->data['sale_type'] = $this->input->post('sale_type');
            $this->data['peperType'] = $peperType;
      ...
      //  $this->sma->generate_pdf($html, $name);
        $this->sma->generatecustom_pdf($html, $name, false,"0",$peperType);
    ...
    public function add($quote_id = null)
    {
        ...
        // กรณีที่มีการตรวจสอบแล้วผ่านทั้งหมด
        if ($this->form_validation->run() == true) {
            // ประเภทของการบันทึก เงินสด เงินเชื่อ
            if($_POST['sale_type']=='1'){
                $reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('LC');
            }else{
                $reference = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('LV');
            }
            //กรณีที่เป็นระดับ Admin และ Owner แก้ไขวันที่ได้
            ...
            $warehouse_id = $this->input->post('warehouse');//รหัสคลังสินค้า
            $customer_id = $this->input->post('customer');//รหัสลูกค้า
            $biller_id = $this->input->post('biller');//รหัสบิล
            $total_items = $this->input->post('total_items');// จำนวนทั้งหมด
            $sale_status = $this->input->post('sale_status');//สถานะการขาย
            $payment_status = $this->input->post('payment_status');// สถานะการจ่ายเงิน
            $payment_term = $this->input->post('payment_term');//รอบการชำระ เครดิจการชำระเงิน
            //วันที่ครบกำหนดการชำระเงิน
            $due_date = $payment_term ? date('Y-m-d', strtotime('+' . $payment_term . ' days', strtotime($date))) : null;
            //ค่าขนส่ง
            $shipping = $this->input->post('shipping') ? str_replace(",","",$this->input->post('shipping')): 0;
            //รายละเอียดของลูกค้า
            ...
            //รายละเอียดของที่อยู่ในการออกบิล
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_type = $_POST['product_type'][$r];//ประเภทสินค้า
                $item_code = $_POST['product_code'][$r];//รหัสสินค้า
                $item_name = $_POST['product_name'][$r];//ชื่อสินคัา
                // ออฟชั่นเพิ่มเติม
                ...
                // ราคาต่อหน่วยจริงๆ
                $real_unit_price = str_replace(",","",$this->sma->formatDecimal($_POST['real_unit_price'][$r])) ;
                // ราคาต่อหน่วย
                $unit_price = $this->sma->formatDecimal(str_replace(",","",$_POST['unit_price'][$r]));
                // จำนวนสินค้า
                $item_unit_quantity = $_POST['quantity'][$r];
                // รหัสสินค้า
                $item_serial = isset($_POST['serial'][$r]) ? $_POST['serial'][$r] : '';
                // ภาษีในตัวสินค้า
                $item_tax_rate = isset($_POST['product_tax'][$r]) ? str_replace(",","",$_POST['product_tax'][$r]) : null;
                $item_discount = isset($_POST['product_discount'][$r]) ? str_replace(",","",$_POST['product_discount'][$r]) : null;
                $item_unit = $_POST['product_unit'][$r];
                $item_quantity = str_replace(",","",$_POST['product_base_quantity'][$r]);
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                   ...
                   // echo $item_net_price."*".$item_unit_quantity."=".($item_net_price * $item_unit_quantity);
                   // echo "<br/>";
            ...
            $order_discount = $this->site->calculateDiscount(str_replace(",","",$this->input->post('order_discount')), ($total + $product_tax));
            $total_discount = $this->sma->formatDecimal(($order_discount), 4);
            $order_tax = $this->site->calculateOrderTaxSale(str_replace(",","",$this->input->post('order_tax')), ($total - $total_discount));
            ...
            $data = array('date' => $date,
                ...
                'order_discount_id' => str_replace(",","",$this->input->post('order_discount')),
                'order_discount' => $order_discount,
                'total_discount' => $total_discount,
                'product_tax' => $product_tax,
                'order_tax_id' => str_replace(",","",$this->input->post('order_tax')),
                'order_tax' => $order_tax,
                'total_tax' => $total_tax,
                'shipping' => $this->sma->formatDecimal(str_replace(",","",$shipping)),
            ...
            if ($payment_status == 'partial' || $payment_status == 'paid') {
                if ($this->input->post('paid_by') == 'deposit') {
                    if ( ! $this->site->check_customer_deposit($customer_id, str_replace(",","",$this->input->post('amount-paid')))) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $sale_id = $this->sales_model->addSale($data, $products, $payment);

            $this->session->set_flashdata('message', lang("sale_added"));
            admin_redirect("sales/view/{$sale_id}/1/?sale_type=".$this->input->post('sale_type'));
        } else {
            //print_r($_GET['sale_type']);
            if ($quote_id || $sale_id) {
                if ($quote_id) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->input->get('id')) { // รหัสใบขาย
            $id = $this->input->get('id');
        }
        $inv = $this->sales_model->getInvoiceByID($id);//ดึงข้อมูลใบขาย
        ...
        if ($this->form_validation->run() == true) {  //หากผ่านการตรวจสอบให้ทำการอัพเดตได้

            $reference = $this->input->post('reference_no'); // เลขที่ใบตรวจสอบ
            if ($this->Owner || $this->Admin) {
                $date = $this->sma->fld(trim($this->input->post('date')));
            } else {
                $date = $inv->date;
            ...
            $warehouse_id = $this->input->post('warehouse'); //รหัสสต๊อกสินค้า
            $customer_id = $this->input->post('customer');  // ลูกค้า
            $biller_id = $this->input->post('biller');  // รหัสเอกสาร
            $total_items = $this->input->post('total_items'); //จำนวนไอเท็ม
            $sale_status = $this->input->post('sale_status'); // สถานะการขาย
            $payment_status = $this->input->post('payment_status'); // 
            $payment_term = $this->input->post('payment_term'); // เทอมการจ่ายเงิน
            $due_date = $payment_term ? date('Y-m-d', strtotime('+' . $payment_term . ' days', strtotime($date))) : null;// วันที่ครบกำหนด
            $shipping = $this->input->post('shipping') ? $this->input->post('shipping') : 0;  //ค่าขนส่ง
            $customer_details = $this->site->getCompanyByID($customer_id); //ข้อมูลบริษัทลูกค้า
            $customer = !empty($customer_details->company) && $customer_details->company != '-'  ? $customer_details->company : $customer_details->name;
            $biller_details = $this->site->getCompanyByID($biller_id); //รายละเอียดผู้ขาย
            $biller = !empty($biller_details->company) && $biller_details->company != '-' ? $biller_details->company : $biller_details->name;
            $note = $this->sma->clear_tags($this->input->post('note'));
            $staff_note = $this->sma->clear_tags($this->input->post('staff_note'));
            ...
            $total_discount = $this->sma->formatDecimal(($order_discount), 4);
            $order_tax = $this->site->calculateOrderTaxSale($this->input->post('order_tax'), ($total - $total_discount));
           // echo $order_tax."<br/>";
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            admin_redirect($inv->pos ? 'pos/sales' : 'sales?sale_type='.$this->input->post('sale_type'));
        } else {
    ...
    public function return_sale($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return)) {
            ...
            admin_redirect($sale->pos ? "pos/sales" : "sales?sale_type=".$this->input->post('sale_type'));
        } else {
