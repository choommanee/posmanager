...
class Reports extends MY_Controller
{
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->datatables
                ...
                COALESCE( PSales.soldQty, 0 ) soldQty,
                CONCAT(COALESCE( PSales.soldQty, 0 ), '__', COALESCE( PSales.totalSale, 0 )) as sold,
                COALESCE( PCosts.purchasedQty, 0 ) - COALESCE( PSales.soldQty, 0 ) as qtytotal,
    ...
    function sales()
    {
        if (!$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
            ...
            //echo $this->Owner."=".$user = $this->session->userdata('user_id');
            $Owner=1;
        }
    ...
    function getSalesReport($pdf = NULL, $xls = NULL,$pid=null)
    {
        ...
        if ($start_date) {
            $start_date = trim($this->sma->fld($start_date));
        ...
        if ($xls) {
            ...
            if($start_date == $end_date){
                if ($start_date) {
                    $this->db->where('sales.date LIKE \''.$start_date.'%\'');
                }
            }else{
                if ($start_date) {
            ...
            $this->db
                ->select("date, reference_no, biller, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '  ', ROUND(" . $this->db->dbprefix('sale_items') . ".quantity,2), '  ',".$this->db->dbprefix('units').".name) SEPARATOR '<br/>') as iname, grand_total, paid, payment_status,created_by", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
           ...
           // echo $this->db->last_query();
            ...
            if (!empty($data)) {
                $this->data['rows'] = $data;
                $html_data = $this->load->view($this->theme . 'reports/sale_pdf', $this->data, true);
                $this->sma->generate_pdf($html_data, "ข้อมูลการขาย");
            }


        }elseif ($this->input->get('pid')==1){

            $si = "( SELECT sale_id, product_id, serial_no, {$this->db->dbprefix('sale_items')}.quantity  as item_nane from {$this->db->dbprefix('sale_items')} ";
    ...
    function vatpurchases()
    {
        $this->sma->checkPermissions('purchases');
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['users'] = $this->reports_model->getStaff();
        $this->data['warehouses'] = $this->site->getAllVatWarehouses();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('vatpurchases_report')));
        $meta = array('page_title' => lang('vatpurchases_report'), 'bc' => $bc);
        $this->page_construct('reports/vatpurchases', $meta, $this->data);
    }
    ...
    function getPurchasesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->setTitle('รายงานภาษาซื้อ');
                
                
                $this->excel->getActiveSheet()->SetCellValue('A1'.$rowHeader, lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1'.$rowHeader, lang('reference_no'));
                $this->excel->getActiveSheet()->SetCellValue('C1'.$rowHeader, lang('warehouse'));
                $this->excel->getActiveSheet()->SetCellValue('D1'.$rowHeader, lang('supplier'));
                $this->excel->getActiveSheet()->SetCellValue('E1'.$rowHeader, lang('product_qty'));
                $this->excel->getActiveSheet()->SetCellValue('F1'.$rowHeader, lang('grand_total'));
                $this->excel->getActiveSheet()->SetCellValue('G1'.$rowHeader, lang('paid'));
                $this->excel->getActiveSheet()->SetCellValue('H1'.$rowHeader, lang('balance'));
                $this->excel->getActiveSheet()->SetCellValue('I1'.$rowHeader, lang('status'));
    ...
    function getVatPurchasesReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('purchases', TRUE);
        ...
        if ($xls) {
            ...
            $this->db
                ->select("" . $this->db->dbprefix('purchases') . ".date, reference_no, supplier,".$this->db->dbprefix('companies').".address, vat_no,grand_total,total_tax, paid, " . $this->db->dbprefix('purchases') . ".status", FALSE)
                ->from('purchases')
                ->join('purchase_items', 'purchase_items.purchase_id=purchases.id', 'left')
                ->join('warehouses', 'warehouses.id=purchases.warehouse_id', 'left')
            ...
            if (!empty($data)) {
                ...
                $rowHeader =5;
                $this->excel->getActiveSheet()->getStyle("A1:I1")->getFont()->setSize(16);
                $this->excel->getActiveSheet()->getStyle("A1:I1")->applyFromArray($style);
                
                $this->excel->getActiveSheet()->getStyle("A2:I2")->getFont()->setSize(16);
                $this->excel->getActiveSheet()->getStyle("A2:I2")->applyFromArray($style);
                $this->excel->getActiveSheet()->getStyle("A3:I3")->getFont()->setSize(16);
                $this->excel->getActiveSheet()->getStyle("A3:I3")->applyFromArray($style);
                $this->excel->getActiveSheet()->getStyle("A4:I4")->getFont()->setSize(16);
                ...
                $this->excel->getActiveSheet()->SetCellValue('A1', "ข้อมูลภาษีซื้อ");
                $this->excel->getActiveSheet()->SetCellValue('A2', "บริษัทแหลมฉบัง โฮมมาร์ท จำกัด");
                $this->excel->getActiveSheet()->SetCellValue('A3', "88/88  ม.10 ต.ทุ่งสุขลา  อ.ศรีราชา  จ.ชลบุรี  20230 ");
                $this->excel->getActiveSheet()->SetCellValue('A4', "เลขประจำตัวผู้เสียภาษี : 0205560022894");
                $this->excel->getActiveSheet()->setTitle(lang('purchase_report'));
                $this->excel->getActiveSheet()->SetCellValue('A'.$rowHeader, lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B'.$rowHeader, lang('reference_no'));
                $this->excel->getActiveSheet()->SetCellValue('C'.$rowHeader, lang('vat_no'));
                $this->excel->getActiveSheet()->SetCellValue('D'.$rowHeader, lang('supplier'));
                $this->excel->getActiveSheet()->SetCellValue('E'.$rowHeader, "ที่อยู่");
                $this->excel->getActiveSheet()->SetCellValue('F'.$rowHeader, "ยอดเงินรวม");
                $this->excel->getActiveSheet()->SetCellValue('G'.$rowHeader, 'ภาษีมูลค่าเพิ่ม');
                $this->excel->getActiveSheet()->SetCellValue('H'.$rowHeader, lang('paid'));
                $this->excel->getActiveSheet()->SetCellValue('I'.$rowHeader, lang('balance'));
                ...
                foreach ($data as $data_row) {
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->reference_no);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->vat_no);
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, $data_row->supplier);
                    ...
                    $total += $data_row->grand_total;
                    $total_vat += $data_row->total_tax;
                    $paid += $data_row->paid;
                    $balance += ($data_row->grand_total - $data_row->paid);
                    $row++;
                }
                $this->excel->getActiveSheet()->getStyle("E" . $row. ":F" .$row. ":G" . $row.":H".$row)->getBorders()
                    ->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
                $this->excel->getActiveSheet()->SetCellValue('F' . $row, $total_vat);
            ...
            if (!empty($data)) {
                $this->data['rows'] = $data;
                $html_data = $this->load->view($this->theme . 'reports/vatpurchases_pdf', $this->data, true);
                 $this->sma->generate_pdf($html_data, $name);
            }
        } else {
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT({$this->db->dbprefix('purchases')}.date, '%Y-%m-%d %T') as date, reference_no, supplier, vat_no,grand_total,total_tax, paid, (grand_total-paid) as balance, {$this->db->dbprefix('purchases')}.status, {$this->db->dbprefix('purchases')}.id as id", FALSE)
                ->from('purchases')
                ->join($pi, 'FPI.purchase_id=purchases.id', 'left')
                ->join('companies', 'companies.id=purchases.supplier_id', 'left')
                ->join('warehouses', 'warehouses.id=purchases.warehouse_id', 'left');
    ...
    function vatpayments()
    {
        ...
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $this->data['pos_settings'] = POS ? $this->reports_model->getPOSSetting('biller') : FALSE;
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('vatpayments_report')));
        $meta = array('page_title' => lang('payments_report'), 'bc' => $bc);
        $this->page_construct('reports/vatpayments', $meta, $this->data);
    }
    
    function getVatPaymentsReport($pdf = NULL, $xls = NULL)
    {
        $this->sma->checkPermissions('payments', TRUE);
        ...
        if ($xls) {
            ...
            $this->db
                ->select("" . $this->db->dbprefix('payments') . ".date, " . $this->db->dbprefix('sales') . ".reference_no as sale_ref, paid_by, amount," . $this->db->dbprefix('sales') . ".order_tax, type")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
            ...
            $this->db
                ->select("DATE_FORMAT({$this->db->dbprefix('payments')}.date, '%Y-%m-%d %T') as date , " . $this->db->dbprefix('sales') . ".reference_no as sale_ref, paid_by," . $this->db->dbprefix('sales') . ".customer ,".$this->db->dbprefix('companies').".address, ".$this->db->dbprefix('companies').".vat_no, amount," . $this->db->dbprefix('sales') . ".order_tax, type, {$this->db->dbprefix('payments')}.id as id")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
                ->join('companies', 'companies.id=sales.customer_id', 'left')
                ->group_by('payments.id')
                ->order_by('sales.id asc ,sales.date asc');
            ...
            if (!empty($data)) {
                $this->data['rows'] = $data;
                $html_data = $this->load->view($this->theme . 'reports/vatpayment_pdf', $this->data, true);
                $this->sma->generate_pdf($html_data, $name);
            }
        } else {
    ...
    function getAdjustmentReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            if ($product || $serial) { $si .= " WHERE "; }
            if ($product) {
                $si .= " {$this->db->dbprefix('adjustment_items')}.product_id = {$product} ";
            }
            if ($product && $serial) { $si .= " AND "; }
            if ($serial) {
                $si .= " {$this->db->dbprefix('adjustment_items')}.serial_no LIKe '%{$serial}%' ";
            }
